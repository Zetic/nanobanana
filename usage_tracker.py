...
class UsageTracker:
    ...
    def _get_current_cycle_key(self) -> str:
        """
        Get the current daily key based on date.
        Usage resets at midnight (00:00) each day.
        Returns format: "2024-01-15"
        """
        now = datetime.now()
    ...
    def _get_next_reset_timestamp(self) -> int:
        """Get the timestamp for the next daily reset (midnight)."""
        now = datetime.now()
        today = now.date()
    ...
    def record_usage(self, user_id: int, username: str, prompt_tokens: int, 
        ...
        with self._lock:
            ...
            # Track images per day if any were generated
            if images_generated > 0:
                current_day = self._get_current_cycle_key()
                if "daily_images" not in user_data:
                    user_data["daily_images"] = {}
                user_data["daily_images"][current_day] = user_data["daily_images"].get(current_day, 0) + images_generated
    ...
    def get_daily_image_count(self, user_id: int) -> int:
        ...
        with self._lock:
            ...
            return daily_images.get(current_day, 0)
    ...
    def reset_daily_usage(self, user_id: int) -> bool:
        """
        Reset today's image usage for a specific user.
        ...
        with self._lock:
            ...
            user_data["daily_images"][current_day] = 0
            
            self._save_usage_data(data)
            logger.info(f"Reset daily usage for user {user_id} (day: {current_day})")
            return True
