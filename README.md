...
- **OpenAI Image Command**: `/gpt-image` supports prompt-only and prompt+image workflows using `gpt-image-2` (`quality="medium"` for prompt-only generation; omitted for edits with input images)
